...
import {
  getInstanceNetInfoObserver,
  notificationsType,
} from '~/store/service/netInfoObserverService';
...
export function* signIn({payload}) {
  try {
    ...
    if (!netState.isConnected) {
      Alert.alert(Messenger.MSG019, Messenger.MSG020);
      return;
    }
    ...
    Alert.alert(Messenger.MSG021, err.message);
...
export function* signUp({payload}) {
  try {
    ...
    Alert.alert(Messenger.MSG022, Messenger.MSG020);
    yield put(signFailure());
  }
}
...
export function* updateStudantProfil({payload}) {
  ...
  let isConnected = yield NetInfo.fetch().then(
    stateNet => stateNet.isConnected,
  );
  ...
  let response = yield api
    .put(`/student/${payload.profile._id}`, {
  ...
  if (response != null) {
    Alert.alert(Messenger.MSG000, Messenger.MSG023);
  ...
  switch (response.status) {
    case 400:
      if (response.message === 'invalid password') {
        Alert.alert('Falha na ataulização do perfil.', 'Senha inválida.');
        break;
      }
  }
